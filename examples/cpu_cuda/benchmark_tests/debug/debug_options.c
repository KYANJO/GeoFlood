...
static void *
debug_register (user_options_t* user, sc_options_t * opt)
{
    /* [user] User options */
...
debug_check (user_options_t *user)
{
    return FCLAW_NOEXIT;
}

static void
debug_destroy(user_options_t *user)
{
    /* Nothing to destroy */
}
...
options_register (fclaw_app_t * app, void *package, sc_options_t * opt)
{
    ...
    return debug_register(user,opt);
}
...
user_options_t* debug_get_options(fclaw2d_global_t *glob)
{
    user_options_t* user = (user_options_t*) fclaw_pointer_map_get(glob->options,"debug");
...
{
    FCLAW_ASSERT(fclaw_pointer_map_get(glob->options,"debug") == NULL);
    fclaw_pointer_map_insert(glob->options,"debug",user,NULL);
}