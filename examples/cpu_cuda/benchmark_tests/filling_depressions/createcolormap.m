...
if ncolor > 3
    ...
    n_each = round((n-rem(n,ncolor-1))/ncolor);
    cmap = cell(ncolor,1);
    nsum = 0;
    for i = 1:ncolor-1
        if i == ncolor -1
            ne  = n_each + rem(n,ncolor-1);
        else
            ne  = n_each;
        end
        cmap{i} = createcolormap(ne, varargin{i+offset_color}, varargin{i+1+offset_color});
        nsum = nsum + ne;
    end
    % if nsum ~= n
        % fprintf('n = %d  nsum = %d n_each =%d\n', n,nsum,n_each)
        % error ("wrong sum")
    % end
    cmap = vertcat(cmap{:});
    return
