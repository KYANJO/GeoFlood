        ...
        ! if 
            ...
            do ibc=1,mbc
                    ...
                    if (q(1,1,j) < dry_tolerance) then
                        h_0 = max((hu_0/sqrt(grav))**(2.0d0/3.0d0), 0.001d0) 
                        q(1,1-ibc,j) = h_0
                        q(2,1-ibc,j) = hu_0
                        q(3,1-ibc,j) = 0.0d0
                    else 
                        ...
                        if (hu_0 .ne. 0.0d0) then
                            call newton_raphson(h_0,hu_0,q(1,1,j),u_1,grav)
                            if (h_0 > q(1,1,j)) then
                                call two_shock(h_0,hu_0,q(1,1,j),u_1,grav)
                            end if
                            q(1,1-ibc,j) = h_0
                            q(2,1-ibc,j) = hu_0
                            q(3,1-ibc,j) = 0.0d0
...
subroutine newton_raphson(h0,hu0,h1,u1,g)
    ...
    ! xn = h1
    do i = 1, max_iter
        fxn = func(hu0,xn,h1,u1,g)
        if (abs(fxn) < tol) then
            h0 = xn
            return 
        end if
        dfxn = dfunc_h0(hu0,xn,h1,u1,g)
...
real(kind=8) function func(hu0,h0,h1,u1,g)
    implicit none
...
!  given hu0
real(kind=8) function dfunc_h0(hu0,h0,h1,u1,g)
    implicit none
    real(kind=8) :: hu0,h0,h1,u1
    real(kind=8) :: g
...
subroutine two_shock(h0,hu0,hr,ur,g)
    implicit none
    ...
    ! xn = x0
    do i = 1,max_iter
        fxn = two_func(hu0,x0,hr,ur,g)
        dfxn = dtwo_func(hu0,x0,hr,ur,g)
...
real(kind=8) function two_func(hu0,h0,hr,ur,g)
    implicit none
    real(kind=8) :: hu0,h0,hr,ur
    real(kind=8) :: g
...
real(kind=8) function dtwo_func(hu0,h0,hr,ur,g)
    implicit none
    real(kind=8) :: hu0,h0,hr,ur
