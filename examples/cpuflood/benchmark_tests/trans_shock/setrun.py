...
# @version: 1.0
# ------------------------
# Description: Transcritical shock problem
# ------------------------
...
if output_style == 1:
    ...
    n_hours = 200/3600              # 200 seconds     
    

    frames_per_minute = 27   # (90 frames)
...
mi = 1  # Number of x grids per block  <-- mx = mi*mx = 20*10 = 200
...
topo_file = './scratch/TS_topohump.tt3'
#------------------------------
...
#-------------------
    ...
    yupper = 12
    ylower = -12
    outfile= "transcritical.asc"   

    # z = 1.0 # Dry bed  

    topo = lambda x,y: np.where((8 < x) & (x <12), (0.2 - (0.05*(x-10)**2)),0.0)
...
#-------------------
    ...
    nxpoints = 1201
    nypoints = 1201
    xlower = 35   #<-- -6.575 for test 6A and -128 for test 6B
    xupper =  -35  #<-- 0 for test 6A and 0 for test 6B
    yupper = 10      #<-- 5.4 for test 6A and 53 for test 6B
    ylower = -10     #<-- -7.31 for test 6A and -55 for test 6B
    outfile= "init.xyz"   

    qinitB = lambda x,y: np.where(x>0, 0.33, 0.33)
