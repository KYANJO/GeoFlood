...
subroutine data_swap(mx, my, mbc, meqn, maux, qold, qold_transpose, aux, aux_transpose, flag2transpose)
  ...
  integer :: i, j, m
  real(kind=8), intent(inout), target :: qold(:,:,:), qold_transpose(:,:,:)
  real(kind=8), intent(inout), target :: aux(:,:,:), aux_transpose(:,:,:)
  real(kind=8), pointer :: temp(:,:,:)

  ! flag2transpose = 1 if qold is (m,i,j) and 0 if qold is (i,j,m)
  if (flag2transpose == 1) then
    ! Swap qold(m,i,j) to qold(i,j,m)
    temp => qold
    qold_transpose(1-mbc:mx+mbc, 1-mbc:my+mbc, :) = transpose(temp, [3,1,2])

    ! Swap aux(m,i,j) to aux(i,j,m)
    temp => aux
    aux_transpose(1-mbc:mx+mbc, 1-mbc:my+mbc, :) = transpose(temp, [3,1,2])
  else
    ! Swap qold(i,j,m) to qold(m,i,j)
    temp => qold
    qold_transpose(:,1-mbc:mx+mbc, 1-mbc:my+mbc) = transpose(temp, [3,1,2])
   
    ! Swap aux(i,j,m) to aux_transpose(m,i,j)
    temp => aux
    aux_transpose(:,1-mbc:mx+mbc, 1-mbc:my+mbc) = transpose(temp, [3,1,2])
  end if
...
! program test
!   implicit none
!   integer, parameter :: mx= 3, my=3, mbc=1, meqn=3, maux=2
!   real(kind=8) :: qold(meqn,1-mbc:mx+mbc,1-mbc:my+mbc)
!   real(kind=8) :: qold_transpose(1-mbc:mx+mbc,1-mbc:my+mbc,meqn)
!   real(kind=8) :: aux(maux,1-mbc:mx+mbc,1-mbc:my+mbc)
!   real(kind=8) :: aux_transpose(1-mbc:mx+mbc,1-mbc:my+mbc,maux)

!   ! Fill qold with random values
!   call random_seed()
!   call random_number(qold)

!   write(*,*) "qold(m,i,j) = ",qold
