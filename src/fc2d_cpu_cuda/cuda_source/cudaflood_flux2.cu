/*
@desc: Broken into x and y components to reduce on warp divergence and improve performance
...
static
__device__
void cudaclaw_flux2_and_update_x(const int mx,   const int my, 
                                ...
                                double t,double dt)
{
    ...
    for(int thread_index = threadIdx.x; thread_index < num_ifaces; thread_index += blockDim.x)
    {
        ...
        */
        ...
        {
            int I_aux = I + m*zs;
            auxr[m] = aux[I_aux];                    /* Right */
            auxl[m] = aux[I_aux - 1];                /* Left  */
        }
...
static
__device__
void cudaclaw_flux2_and_update_y(const int mx,   const int my, 
                               const int meqn, const int mbc,
                               const int maux, const int mwaves, 
                               const int mwork, int src_term,
                               const int mcapa, const double dry_tol,
                                ...
                                double t,double dt)
{
    ...
    for(int thread_index = threadIdx.x; thread_index < num_ifaces; thread_index += blockDim.x)
    {
        ...
        ---> 0 <= ix < mx + 2*mbc - 2
        ---> 0 <= iy < mx + 2*mbc - 1
        ---> ys+1 <= I < (mx+2*mbc-2)*ys + (mx+2*mbc-1)
        ---> ys+1 <= I <= (mx+2*mbc-3)*ys + (mx+2*mbc-1)

        Example : mbc=2; mx=8; (ix=0,iy=0) --> I = 13  (i=0,j=0)
                               (ix=mx+1,iy=mx) --> I=10*12 + 9 = 129
                               (i=mx+1,j=my)
        */
    ...
    /* ------------------------ Transverse Propagation : Y-faces ---------------------- */
    for(int thread_index = threadIdx.x; thread_index < num_ifaces; thread_index += blockDim.x)
    { 
        int ix = thread_index % ifaces_x;
        int iy = thread_index/ifaces_x;
                                ...
                                double t,double dt)
{
    ...
    double maxcfl_x, maxcfl_y;
    /* ------------ perform normal and transverse updates in the x-direction ----------------- */
    cudaclaw_flux2_and_update_x(mx,my,meqn,mbc,maux,mwaves,mwork,
                                src_term, mcapa, dry_tol,
                                xlower, ylower, dx, dy,
                                qold, aux, fm, fp, gm, gp, amdq_trans, apdq_trans, waves, speeds,   
                                maxcfl_x, rpn2, rpt2, t, dt);
    __syncthreads();

    /* ------------ perform normal and transverse updates in the y-direction ----------------- */
    cudaclaw_flux2_and_update_y(mx,my,meqn,mbc,maux,mwaves,mwork,
                                src_term, mcapa, dry_tol,
                                xlower, ylower, dx, dy,
                                qold, aux, fm, fp, gm, gp, bmdq_trans, bpdq_trans, waves, speeds,
                                maxcfl_y, rpn2, rpt2, t, dt);

    __shared__ double dtdx, dtdy;
    __shared__ int xs, ys, zs;
    /* ------------------------ Compute grid cfl number -------------------------------------- */
    double aggregate = BlockReduce(temp_storage).Reduce(fmax(maxcfl_x,maxcfl_y),cub::Max());
    if (threadIdx.x == 0)
    {
        maxcflblocks[blockIdx.z] = aggregate;
    ...
    /* ------------------------ Wave propagation algorithm update  ---------------------------- */
